...
public class HuggingFaceImageEmbeddingServiceTests
{
    [Fact]
    public async Task EmbedImageAsync_FlatVector_ReturnsVectorAndModel()
    {
        var service = CreateService(_ => JsonResponse(HttpStatusCode.OK, "[0.1,0.2,0.3]"));

        var result = await service.EmbedImageAsync(ImageStream());

        result.Vector.Should().Equal(0.1f, 0.2f, 0.3f);
        result.ModelVersion.Should().Be("sentence-transformers/clip-ViT-B-32");
    }

    [Fact]
    public async Task EmbedImageAsync_BatchedVector_UnwrapsSingleBatch()
    {
        var service = CreateService(_ => JsonResponse(HttpStatusCode.OK, "[[0.4,0.5,0.6]]"));

        var result = await service.EmbedImageAsync(ImageStream());

        result.Vector.Should().Equal(0.4f, 0.5f, 0.6f);
    }
    ...
    public async Task EmbedImageAsync_RateLimitedThenOk_RetriesAndSucceeds()
    {
        ...
        var service = CreateService(_ =>
        {
            ...
            return calls == 1
                ...
                : JsonResponse(HttpStatusCode.OK, "[0.5,0.6]");
        });
    ...
    [Fact]
    public async Task EmbedImageAsync_ModelLoadingThenOk_RetriesAndSucceeds()
    {
        var calls = 0;
        var service = CreateService(_ =>
        {
            calls++;
            return calls == 1
                ? JsonResponse(HttpStatusCode.ServiceUnavailable, """{"error":"Model is currently loading"}""")
                : JsonResponse(HttpStatusCode.OK, "[0.7,0.8]");
        });

        var result = await service.EmbedImageAsync(ImageStream());

        calls.Should().Be(2); // retried once after the 503 cold-start
        result.Vector.Should().Equal(0.7f, 0.8f);
    }
    ...
    private static HuggingFaceImageEmbeddingService CreateService(
        ...
        string apiKey = "test-api-key")
    {
        ...
        factory.Setup(f => f.CreateClient(HuggingFaceImageEmbeddingService.HttpClientName)).Returns(httpClient);

        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["HuggingFace:ApiKey"] = apiKey,
                ["HuggingFace:RetryBaseDelayMs"] = "1", // keep the retry tests fast
            })
            .Build();

        return new HuggingFaceImageEmbeddingService(factory.Object, config, NullLogger<HuggingFaceImageEmbeddingService>.Instance);
    }
