...
public class JinaImageEmbeddingServiceTests
{
    [Fact]
    public async Task EmbedImageAsync_ValidResponse_ReturnsVectorAndModel()
    {
        var service = CreateService(_ => JsonResponse(HttpStatusCode.OK,
            """{"model":"jina-clip-v2","data":[{"index":0,"embedding":[0.1,0.2,0.3]}]}"""));

        var result = await service.EmbedImageAsync(ImageStream());

        result.Vector.Should().Equal(0.1f, 0.2f, 0.3f);
        result.ModelVersion.Should().Be("jina-clip-v2");
    }

    [Fact]
    public async Task EmbedImageAsync_SendsBase64ImageAndBearerToken()
    {
        ...
        var service = CreateService(req =>
        {
            ...
            return JsonResponse(HttpStatusCode.OK, """{"model":"jina-clip-v2","data":[{"embedding":[0.1]}]}""");
        });

        await service.EmbedImageAsync(ImageStream());

        captured!.Headers.Authorization!.ToString().Should().Be("Bearer test-api-key");
        // 0xFF 0xD8 0xFF 0xE0 base64-encodes to /9j/4A==
        capturedBody.Should().Contain("\"image\":\"/9j/4A==\"");
    }
    ...
    [Fact]
    public async Task EmbedImageAsync_EmptyData_Throws()
    {
        var service = CreateService(_ => JsonResponse(HttpStatusCode.OK, """{"model":"jina-clip-v2","data":[]}"""));
    ...
    public async Task EmbedImageAsync_RateLimitedThenOk_RetriesAndSucceeds()
    {
        ...
        var service = CreateService(_ =>
        {
            calls++;
            return calls == 1
                ? JsonResponse(HttpStatusCode.TooManyRequests, """{"detail":"slow down"}""")
                : JsonResponse(HttpStatusCode.OK, """{"model":"jina-clip-v2","data":[{"embedding":[0.5,0.6]}]}""");
        });
    ...
    {
        var service = CreateService(_ => JsonResponse(HttpStatusCode.TooManyRequests, """{"detail":"slow down"}"""));
    ...
    private static JinaImageEmbeddingService CreateService(
        ...
        string apiKey = "test-api-key")
    {
        ...
        factory.Setup(f => f.CreateClient(JinaImageEmbeddingService.HttpClientName)).Returns(httpClient);

        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["Jina:ApiKey"] = apiKey,
                ["Jina:RetryBaseDelayMs"] = "1", // keep the retry tests fast
            })
            .Build();

        return new JinaImageEmbeddingService(factory.Object, config, NullLogger<JinaImageEmbeddingService>.Instance);
    }
